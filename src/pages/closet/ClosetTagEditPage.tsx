...
import { OCR_ITEMS, type ChipKind, type OcrItem } from '@/features/closet/ocrItems';

/**
 * 태그 수정 — 인식 결과의 칩(카테고리/세부/브랜드/해시태그)을 눌러 선택(회색)하고 X로 삭제한다.
 * 확인 → 추가 완료.
 * ※ 확인 버튼 활성 조건(선택/변경)은 디자이너 확인 대기 — 현재는 선택하거나 지우면 활성
 */
const ClosetTagEditPage = () => {
  const navigate = useNavigate();
  const [items, setItems] = useState<OcrItem[]>(OCR_ITEMS);
