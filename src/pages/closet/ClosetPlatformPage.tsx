...
const SlotBox = ({ angle, kind, gid }: { angle: number; kind: 'gray' | 'top' | 'bottom'; gid: string }) => (
...
const ClosetPlatformPage = () => {
  ...
  const selPos = pos(DOT_R, selected.angle);
      ...
      <div className="relative flex flex-col flex-1 min-h-0 bg-white overflow-hidden">
        ...
        >
          ...
          {items.map((it) => {
            ...
            const kind: 'gray' | 'top' | 'bottom' = edge ? (slot < 0 ? 'top' : 'bottom') : 'gray';
            const dotColor = edge ? '#CED1D5' : '#959BA7';
            const dot = pos(DOT_R, it.angle);
            const box = pos(BOX_R, it.angle);
            const label = pos(LABEL_R, it.angle);
            const labelStyle = edge
              ? {
          ...
          >
            <span className="absolute -translate-x-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-[#1F2124]" />
            <span
        ...
        <div className="relative z-10 w-full px-6 pt-3 pb-[calc(40px+env(safe-area-inset-bottom,0px))]">
          {/* 다음 (비활성화) — bg #1F2124 4%, radius 32 */}
          <button
            type="button"
            disabled
            className="w-full h-[58px] rounded-[32px] text-center text-[16px] font-semibold leading-[1.6] tracking-[-0.02em] text-[#1F2124] cursor-not-allowed"
            style={{ backgroundColor: 'rgba(31,33,36,0.04)', boxShadow: '0px 8px 16px 0px rgba(0,0,0,0.08)' }}
          >
            다음
          </button>
        </div>
