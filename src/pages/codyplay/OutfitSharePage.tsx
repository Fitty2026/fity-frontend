...
const OutfitSharePage = () => {
  ...
  const downloadImg = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `fitty-${result.createdAt ?? 'outfit'}.jpg`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
    } catch {
      // CORS 등으로 blob 변환이 막히면 새 탭으로라도 이미지를 연다
  ...
  const shareSns = async () => {
    const shareData = {
      title: 'Fitty 코디',
      text: result?.context ?? 'Fitty에서 만든 코디를 확인해 보세요',
      url: result?.imageUrl ?? window.location.href,
    };
    if (navigator.share) {
      try {
        await navigator.share(shareData);
      } catch {
        // 사용자가 공유 시트를 닫은 경우 포함 — 별도 처리 없음
      }
    } else {
      // Web Share API 미지원 브라우저는 링크 복사로 대체한다
      try {
        await navigator.clipboard.writeText(shareData.url);
        alert('코디 링크가 복사되었습니다.');
      } catch {
  ...
  return (
    <PageLayout showHeader={false} showBottomNav={false}>
      <div className="relative z-10 mt-[32px] mx-[24px] aspect-[327/376]">
        <div
          className={`${location.state?.animateImage ? 'outfit-share-image-enter' : ''} absolute top-0 left-0 w-full aspect-[327/376] overflow-hidden rounded-[24px]`}
        >
          <img
      ...
      <div className={location.state?.animateImage ? 'outfit-share-content-enter' : ''}>
