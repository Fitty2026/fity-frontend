interface PhotoSlotGridProps {
  /** 채워진 사진 objectURL 목록 (앞에서부터 정면→측면→후면 순서로 채움) */
  photos: string[];
  /** 전체 슬롯 수 (기본 3) */
  max?: number;
    ...
    {Array.from({ length: max }).map((_, i) => (
      <div key={i} className="flex flex-col items-center gap-2">
        {i < photos.length ? (
          ...
          <div className="aspect-[3/4] w-full rounded-2xl border-2 border-dashed border-neutral-200" />
        )}
        {/* 아직 못 채운 슬롯의 라벨은 보라색으로 강조 (Figma 비활성 페이지) */}
        {labels && (
          <span className={`text-sm ${i < photos.length ? 'text-neutral-700' : 'text-violet-500'}`}>
            {labels[i]}
          </span>
        )}
      </div>
