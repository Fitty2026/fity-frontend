import { shoppingMallLabel } from '@/features/closet/shoppingMalls';
 ...
 */
const ReceiptCard = ({ item, className = '' }: { item: OcrResult; className?: string }) => (
  ...
  >
    ...
    <div className="rounded-t-[16px] bg-white px-4 pt-8 pb-8" style={scallopStyle}>
      {/* 매장명 — Title/T2 20px Bold, 블록 327×30. 영수증 원문이 있으면 그대로, 없으면 쇼핑몰 이름 */}
      <p className="text-center text-[20px] font-bold leading-[1.5] tracking-[-0.02em] text-[#1F2124]">
        {item.storeText || shoppingMallLabel(item.store)}
      </p>
      ...
      {/* 상품 행 295×66 — 왼쪽 여백 12, 텍스트만 (시안에 썸네일 없음). 라벨 아래 8 */}
