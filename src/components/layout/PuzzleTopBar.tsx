...
const PuzzleTopBar = ({ title, showBack = true, height = 53 }: PuzzleTopBarProps) => {
  const navigate = useNavigate();
  const balance = usePuzzleBalance();

  return (
    <header
      className="relative flex shrink-0 items-center justify-center border-b border-[#B2B8BD] bg-white"
      style={{ height }}
    >
      {showBack ? (
        <button
      ...
      <span
        className="absolute right-6 flex items-center gap-1 whitespace-nowrap text-[12px] font-medium leading-[1.65] tracking-[-0.02em] text-[#1F2124]"
        aria-label={`퍼즐 ${balance ?? 0}개`}
      >
        <PuzzleIcon />
        {balance ?? 0}개
      </span>
